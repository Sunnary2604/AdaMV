...
df_2=pd.read_csv("D:/parse/time_series_covid19_recovered_global.csv")
symbol=[]
date=[]
number=[]
for i in range(df.shape[0]):
    if(df.iloc[i]["location"]=="World"):
        if(df.iloc[i]["date"]=="2021/1/1"):
            break
        number.append(df.iloc[i]["total_cases"]/1000000)
        symbol.append("total cases")
        date.append(df.iloc[i]["date"])
        number.append(df.iloc[i]["total_deaths"]/1000000)
