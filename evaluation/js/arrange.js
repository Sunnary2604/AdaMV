function generate(data)
{
    ...
    {
        font={};
        font.size=Math.round(22-16*(1-(Math.min(data.Modality.Size.width,data.Modality.Size.height)-320)/760));
...
function form_mark(unit,vlSpec)
{
    if(unit.hasOwnProperty("layer"))
    {
        ...
        vlSpec.layer[1].mark.fontSize=0.8*font.size;
...
function drawmap(unit,vlSpec)
{
    ...
    for(let i=0;i<unit.Encoding.length;i++)
    {
        ...
        else
        {
            ...
            vlSpec.config.legend.gradientHorizontalMinLength=Math.min(vlSpec.width,vlSpec.height)/3;
...
function form_encoding(unit,vlSpec)
{
    ...
    else
    {
        ...
        if(unit.Encoding.x.hasOwnProperty("domain"))
        {
            vlSpec.encoding.x.scale={};
            vlSpec.encoding.x.scale.domain=unit.Encoding.x.domain;
        }
        if(unit.Encoding.y.hasOwnProperty("domain"))
        {
            vlSpec.encoding.y.scale={};
            vlSpec.encoding.y.scale.domain=unit.Encoding.y.domain;
        }
