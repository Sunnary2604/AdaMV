...
def initialTreeNode(inputJson, inputDevice):
    TreeNodes  = []
    # Create leaf nodes
...
def createNTree(TreeNodes):
    ...
    while len(copyTreeNodes) != 1:
        ...
        for index1, node1 in  enumerate(copyTreeNodes):
            ...
            if breakflag:
                break
    # Sort child nodes by sortedIndex
    copyTreeNodes[0].child=sorted(copyTreeNodes[0].child,key=lambda x:x.val.sortedIndex,reverse=False)
...
def judgeSmallMultiple(TreeNodes):
    ...
    while len(copyTreeNodes) != 0:
        ...
        for smItemIndex in range(len(sm)):
            ...
            if tempNodes.val.type == smItem.val.type and (tempNodes.val.width - smItem.val.width)<floatE and tempNodes.val.height == smItem.val.height:
...
def spilt_NTree(inputJson, inputDevice):

    [TreeNodes,unSortedTreeNodes] = initialTreeNode(inputJson, inputDevice)
