...
import pandas as pd
import math
df = pd.read_csv("F:/GitHub/LayoutCMV_Code-Paper/show-system/demo1/01-06-2021.csv")
long=[]
lat=[]
number=[]
country=[]
...
for i in range(df.shape[0]):
    ...
    if(i!=0 and (df.iloc[i]["Country_Region"]==df.iloc[i-1]["Country_Region"]) and 
        ...
        total=0
         ...
         (df.iloc[i]["Country_Region"]!=df.iloc[i+1]["Country_Region"]))):
        number.append(df.iloc[i]["Confirmed"])
    if(df.iloc[i]["Whethercapital"]=="Y"):
        long.append(round(df.iloc[i]["Long"],2))
        lat.append(round(df.iloc[i]["Lat"]+1.4,2))
