let oRRadius = 8;
let leftinterfaceORcoordinates = {
  ORcoordinates: [],
  x: [],
  y: [],
  xMap: [],
...
let rightinterfaceORcoordinates = {
  ORcoordinates: [],
  x: [],
  y: [],
  xMap: [],
...
function calinterfaceORcoordinates(div, num, door) {
  ...
  if (door == "#left") {
    leftinterfaceORcoordinates.ORcoordinates[num] = oRcoordinates;
  } else {
    rightinterfaceORcoordinates.ORcoordinates[num] = oRcoordinates;
  }
}
...
function xyinterfaceORcoordinates(
  ...
  if (door == "#left") {
    interfaceORcoordinates = leftinterfaceORcoordinates;
  } else {
    interfaceORcoordinates = rightinterfaceORcoordinates;
  }
...
function operation(div, num, views, door, idDoor, viewport) {
  ...
  for (let item in oRcoordinates) {
    ...
    oR.onmousedown = function (e) {
      ...
      let clickORmapsSet = new Set();
      for (let i = 0; i < clickORmaps.length; i++) {
        clickORmapsSet.add(clickORmaps[i]);
      }
      clickORmaps = Array.from(clickORmapsSet);
      for (let i = 0; i < clickORmaps.length; i++) {
    ...
    document.onmouseup = function (e) {
      if (operationFlag) {
        ...
        for (let i = 0; i < clickORmaps.length; i++) {
          let iArr = clickORmaps[i].split("-");
          let divID = iArr[1];
          clickORmapsSet.add(divID);
        }
        clickORmapsSet = Array.from(clickORmapsSet);
        for (let i = 0; i < clickORmapsSet.length; i++) {
          let divSelects = document.getElementById(
            idDoor + "p" + clickORmapsSet[i]
          );
          subchangedivStyle(
            divSelects,
            clickORmapsSet[i],
          ...
          reviseData(idDoor, divSelects, clickORmapsSet[i], viewport);
        }
        svDisplay(
          document.getElementById(idDoor + "p" + num.toString()),
          viewport
        );
        operationFlag = false;
      }
...
function changedivStyle(
  ...
  let name = "#" + idDoor + "p" + num.toString() + " .chart-wrapper";
  d3.select(name).remove();
}

function subchangedivStyle(div, num, views, door, idDoor, viewport) {
...
function reviseData(idDoor, div, num, viewport) {
  let divTop = Number(div.style.top.slice(0, div.style.top.length - 2));
  let divLeft = Number(div.style.left.slice(0, div.style.left.length - 2));
  let divWidth = Number(div.style.width.slice(0, div.style.width.length - 2));
  let divHeight = Number(
    div.style.height.slice(0, div.style.height.length - 2)
  );
  if (idDoor == "left-middle-content1") {
    data.Interface[num].BoundingBox.Size.width = divWidth / viewport[2];
    data.Interface[num].BoundingBox.Size.height = divHeight / viewport[3];
    data.Interface[num]["BoundingBox"]["CenterPosition"]["x"] =
      (divLeft + divWidth / 2) / viewport[2];
    data.Interface[num]["BoundingBox"]["CenterPosition"]["y"] =
      (divTop + divHeight / 2) / viewport[3];
  } else {
    rightData.Interface[num].BoundingBox.Size.width = divWidth / viewport[2];
    rightData.Interface[num].BoundingBox.Size.height = divHeight / viewport[3];
    rightData.Interface[num]["BoundingBox"]["CenterPosition"]["x"] =
      (divLeft + divWidth / 2) / viewport[2];
    rightData.Interface[num]["BoundingBox"]["CenterPosition"]["y"] =
      (divTop + divHeight / 2) / viewport[3];
  }
}
