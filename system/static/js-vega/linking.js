function form_linking(data,idDoor)
{
    if(data.hasOwnProperty("linking"))
    {
        ...
        info.hl = -1;
        setTimeout(function(){
            info=storeinfo(data,info,idDoor);
        },1500);
        $('div',document).mouseover(function (e)
        {
            id=this.id;
            if (e.target.ariaLabel !== null && id[id.length-2] === "p" &&
            ...
            else if(e.target.ariaLabel === null&&!flag_1)
            {
                for (let i = 0; i < data.linking.views.length; i++)
                {
                    recover(data.linking.views[i], data, info,idDoor);
                }
                flag_1 = true;
            }
        });
...
function recover(num,data,info,idDoor)
{
...
function storeinfo(data,info,idDoor)
{
    for(let i=0;i<data.linking.views.length;i++)
    {
        let selectElement,name;
        ...
        for(let j=0;j<selectElement[0].childNodes.length;j++)
        {
            ...
            {
                info.color[data.linking.views[i].toString()][j]=selectElement[0].childNodes[j].attributes[4].nodeValue;
            }
            else
            {
                info.color[data.linking.views[i].toString()][j]=selectElement[0].childNodes[j].attributes[5].nodeValue;
            }
        }
    }
    return info;
