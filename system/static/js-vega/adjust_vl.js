function adjust_vl(data,views,idDoor,viewport)
{
    ...
    for(let i=0;i<data.Interface.length;i++)
    {
        ...
        adjustment=judge(i,data,idDoor,viewport);
        unit=operation_vl(adjustment,views[i]);
        name = "#"+idDoor+"p" + i.toString();
        if(adjustment.x.iflinebreak)
        {
            if(adjustment.x.ifrotate)
            {
                views[i].config.padding.bottom=-4;
            }
            else
            {
...
function judge(num,data,idDoor,viewport)
{
    ...
    adjustment=check_y(selectElement[1],adjustment,data,viewport);
    width = Math.round(data.Interface[num].BoundingBox.Size.width * viewport[2]);
    height = Math.round(data.Interface[num].BoundingBox.Size.height * viewport[3]);
...
function setValue(adjustment,width,height,unit)
{
    if(adjustment.x.ifrotate)
    {
        ...
        {
            if(adjustment.x.maxlen>0.25*height)
            {
            ...
            {
                adjustment.x.rotation=-90;
            }
        }
    }
    if(adjustment.y.ifrotate)
    {
        if(adjustment.y.maxlen<=0.25*width)
        {
            adjustment.y.ifrotate=false;
        }
        else if(0.707*adjustment.y.maxlen>=0.25*width)
        {
            adjustment.transposition=true;
            adjustment.y.ifrotate=false;
        }
...
function check_y(Element,adjustment)
{
    let max_len=0,str,mid_1,mid_2,flag=true,index;
    str=Element.childNodes[0].attributes[1].value;
    index=str.indexOf(",");
    mid_1=Number(str.substring(index+1,str.length-2));
    for(let i=1;i<Element.childNodes.length;i++)
    {
        ...
        if(flag&&Element.childNodes[i].attributes[5].value==="1")
        {
            flag=false;
            str=Element.childNodes[i].attributes[1].value;
            index=str.indexOf(",");
            mid_2=Number(str.substring(index+1,str.length-2));
        }
    }
    adjustment.y.maxlen=max_len;
    if(Math.abs(mid_2-mid_1)<=font.size)
    {
        adjustment.transposition=true;
    }
    else
