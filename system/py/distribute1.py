        ...
        for pRes in copyPermutationRes:
            ...
            for pResIndex in range(len(pRes)):
                ...
                if len(outputDevicesPRes) == 1:
                    pRes[pResIndex][0] = sa(outputDevices[0], pRes[pResIndex][0], weight, 'root', viewTypeContrains)
                else:
                    pRes[pResIndex][0] = sa(outputDevices[oDPRes[pResIndex]], pRes[pResIndex][0], weight, 'root', viewTypeContrains)
                # Hierarchical traversal
                queue = [pRes[pResIndex][0]]
                layerRoots = []
                flag = 0
                while len(queue) != 0:
                    queueNum = len(queue)
