function generate(data)
{
    ...
    for (let i = 0; i < data.Interface.length; i++)
    {
        font={};
        //font.size = 13;
        font.size=Math.round(22
            -10*(1-(Math.min(data.Modality.Size.width,data.Modality.Size.height)-320)/760));
        font.gap=Math.round(0.1*font.size);
...
function form_mark(unit,vlSpec)
{
    if(unit.hasOwnProperty("layer"))
    {
        ...
        vlSpec.layer[1].mark.radius=Math.min(vlSpec.height,vlSpec.width)/3.5;
...
function form_config(unit,vlSpec)
{
    ...
    if(unit.Encoding.hasOwnProperty("color")||unit.Encoding.hasOwnProperty("size"))
    {
        ...
        else
        {
            if(unit.hasOwnProperty("transform"))
            {
                vlSpec.config.legend.titlePadding=16;
