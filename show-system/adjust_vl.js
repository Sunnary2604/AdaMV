...
function judge(num,data)
{
    ...
    adjustment=check_y(selectElement[1],adjustment);
...
function setValue(adjustment,width,height,unit)
{
    if(adjustment.x.ifrotate)
    {
        if(0.707*adjustment.x.maxlen>0.3*height)
        {
        ...
        {
            if(adjustment.x.maxlen>0.3*height)
            {
            ...
            if(unit.ViewType.type==="line"){
                console.log('line',adjustment.x.rotation);
            }
        }
    }
    if(adjustment.y.ifrotate)
    {
        if(adjustment.y.maxlen<=0.3*width)
        {
            adjustment.y.ifrotate=false;
        }
        else if(0.707*adjustment.y.maxlen>=0.3*width)
        {
            adjustment.transposition=true;
            adjustment.y.ifrotate=false;
        }
...
function check_x(unit,Element,adjustment)
{
    ...
    console.log(unit.ViewType.type, adjustment.x.ifrotate)
    if(adjustment.x.ifrotate)
    {
        adjustment.x.iflinebreak=false;
