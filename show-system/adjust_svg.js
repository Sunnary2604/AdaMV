function adjust_svg(views) {
  for (let i = 0; i < views.length; i++) {
    if (
      views[i].hasOwnProperty("projection") ||
      Object.keys(views[i]).length === 0 ||
      views[i].hasOwnProperty("layer")
    ) {
      continue;
    }
    // ame = "#p" + i.toString()+" .marks";n
    // div=$(name);
    // width = views[i].width;
    // height = views[i].height;
    // div[0].attributes[4].value=width.toString()+"px";
    // div[0].attributes[5].value=height.toString()+"px";
...
function correct_label(num) {
  ...
  for (let i = 0; i < selectElement[0].childNodes.length; i = i + gap) {
    if (i + gap < selectElement[0].childNodes.length) {
      ...
      textlength_2 =
        selectElement[0].childNodes[i + gap].textLength.baseVal.value;
      //textlength_1=selectElement[0].childNodes[i].innerHTML.length*12;
      //textlength_2=selectElement[0].childNodes[i+1].innerHTML.length*12;
      if (
        (textlength_1 + textlength_2) / 2 >
        Math.abs(mid_2 - mid_1) - font.gap
      ) {
        if (flag[i] === false) {
...
function formarray(arr_ori, gap) {
  ...
  while (i < arr_ori.length) {
    if (
      i < arr_ori.length - 1 &&
      arr_ori[i].length + arr_ori[i + 1].length <= Math.round(gap / 12) - 4
    ) {
...
function correct_AxistitleandPielabel(views) {
  ...
  for (let i = 0; i < views.length; i++) {
    ...
    name = "#p" + i.toString() + " .role-axis-title";
    let selectElement = $(name);
    if (views[i].mark.type === "rect") {
      selectElement[0].childNodes[0].textContent =
        selectElement[0].childNodes[0].textContent.replace(/\(binned\)/, "");
      selectElement[0].childNodes[1].textContent =
        selectElement[1].childNodes[0].textContent.replace(/\(binned\)/, "");
    }
    if (!views[i].config.axisX.hasOwnProperty("title")) {
      if (
        selectElement[0].childNodes[0].textLength.baseVal.value > views[i].width
      ) {
        name = "#p" + i.toString() + " .role-axis-label";
        selectLabel = $(name);
    ...
    let j = 0;
    if (
      !views[i].config.axisX.hasOwnProperty("title") &&
      views[i].encoding.x.type !== "temporal"
    ) {
      j++;
    }
    if (!views[i].config.axisY.hasOwnProperty("title")) {
      if (
        selectElement[j].childNodes[0].textLength.baseVal.value >
        views[i].height
      ) {
        name = "#p" + i.toString() + " .role-axis-label";
        selectLabel = $(name);
...
function correct_pielabel(num) {
  let name = "#p" + num.toString() + " .layer_1_marks";
  let selectElement = $(name);
  for (let i = 0; i < selectElement[0].childNodes.length; i++) {
    selectElement[0].childNodes[i].attributes[7].value = "#000";
  }
}
function deleteminus(num) {
  let name, selectLabel;
  name = "#p" + num.toString() + " .role-axis-label";
  selectLabel = $(name);
  for (let i = 0; i < selectLabel[0].childNodes.length; i++) {
    if (selectLabel[0].childNodes[i].attributes[5].value === "1") {
      selectLabel[0].childNodes[i].innerHTML = selectLabel[0].childNodes[
        i
      ].innerHTML.replace("−", "");
    }
  }
